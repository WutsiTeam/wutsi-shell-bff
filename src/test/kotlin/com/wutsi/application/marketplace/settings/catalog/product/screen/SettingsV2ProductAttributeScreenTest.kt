...
internal class SettingsV2ProductAttributeScreenTest : AbstractSecuredEndpointTest() {
    @LocalServerPort
    val port: Int = 0
    ...
    private fun url(name: String, action: String = "") =
        "http://localhost:$port${Page.getSettingsCatalogUrl()}/product/editor$action?id=${product.id}&name=$name"
    ...
    @Test
    fun `title`() =
        assertEndpointEquals("/marketplace/settings/catalog/product/screens/editor-title.json", url("title"))

    @Test
    fun `summary`() =
        assertEndpointEquals("/marketplace/settings/catalog/product/screens/editor-summary.json", url("summary"))

    @Test
    fun `description`() =
        assertEndpointEquals(
            "/marketplace/settings/catalog/product/screens/editor-description.json",
            url("description")
        )

    @Test
    fun `quantity`() =
        assertEndpointEquals("/marketplace/settings/catalog/product/screens/editor-quantity.json", url("quantity"))

    @Test
    fun `price`() =
        assertEndpointEquals("/marketplace/settings/catalog/product/screens/editor-price.json", url("price"))
