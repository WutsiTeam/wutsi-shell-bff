...
internal class SettingsProductEditorScreenTest : AbstractEndpointTest() {
    ...
    @Test
    fun index() = assertEndpointEquals("/store/screens/settings/product/editor.json", url)

    @Test
    fun `digital product enabled`() {
        doReturn(true).whenever(togglesProvider).isDigitalProductEnabled()

        assertEndpointEquals("/store/screens/settings/product/editor-digital-product-enabled.json", url)
    }
}
